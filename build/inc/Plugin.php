...
class Plugin {
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The plugin component %1$s does not implement the %2$s interface.', 'tecut' ),
						gettype( $component ),
	 ...
	 *
	 * Calling `wp_tecut()` is a short-hand for calling this method on the main plugin instance. The instance then allows
	...
	public function component( $slug ) {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No plugin component with the slug %s exists.', 'tecut' ),
					$slug
				)
			);
		}
